...
def vitesse_radiale_point_b0(x, y, R, P_deg, B0_deg, V_eq=1.865):
    """
    Calcule la vitesse radiale projetée (en km/s) au point (x, y)
    sur le disque solaire, en tenant compte de l'angle polaire P
    et de la latitude B0.

    Paramètres :
    - x, y : coordonnées du point (origine au centre du disque, en pixels)
    - R : rayon solaire en pixels
    - P_deg : angle polaire (°)
    - B0_deg : latitude héliographique du centre du disque (°)
    - V_eq : vitesse équatoriale en km/s (défaut 1.865 km/s)

    Retour :
    - v_r : vitesse radiale en km/s (0 si hors du disque)
    
    # Exemple
    vr = vitesse_radiale_point_b0(x=150, y=50, R=350, P_deg=23, B0_deg=7) 
    print(f"Vitesse radiale : {vr:.3f} km/s »)
    ...
    # Rotation inverse (coordonnées dans le repère solaire)
    x_rot = x * np.cos(P) + y * np.sin(P)
    y_rot = -x * np.sin(P) + y * np.cos(P)
    ...
    if r >= 1.0:
        return 0.0  # En dehors du disque

    rho = np.arcsin(r) if r > 1e-6 else 0.0

    # Calcul de sin(B)
    sinB = np.cos(rho) * np.sin(B0)
    if r > 1e-6:
        sinB += (y_n / r) * np.sin(rho) * np.cos(B0)
    sinB = np.clip(sinB, -1.0, 1.0)
    cosB = np.sqrt(1 - sinB**2)

    # Calcul de sin(L)
    if r > 1e-6:
        sinL = (x_n / r) * np.sin(rho) / cosB
    else:
        sinL = 0.0
...
def detect_bord (img, axis, offset, flag_disk):
    ...
    if axis==1:
        ...
        if debug:
            ...
            plt.show()
            print("Position Y des limbes y1, y2 :"+ str(a1)+','+ str(a2))
    else:
...
def detect_edge (myimg,zexcl, crop, disp_log):
    ...
    if abs(crop) >=100 : # fort tilt
        ...
        # modif du 17 mai 2025 profil Aguerre 8 bits
        dy = int(abs(crop)/2 *((rayon*2)/iw)) 
        zexcl=0
    ...
    ze1=ze 
    ze2=ze 
    # zone reduite et lissage gaussien adaptatif
    # modif du 17 mai 2025 profil Aguerre 8 bits
    xg=max(0,x1-100)
    xd= min(x2+100,iw)
    kernel = int((x2-x1)/100)
    if kernel <11 :
        kernel=11
    else :
        if kernel % 2 == 0 :
    ...
    for i in range(y1+ze1,y2-ze2):
        ...
        li_filter=gaussian_filter1d(li, kernel)
    ...
    for s in range (0,2) :
        ...
        for i in range(0,2) :
            ...
            # protu depasse du disque
            # filament zone noire interieur du disque
                
            for k in range(0,len(fp)-1) :   
                if i==0 :
                    #if (fp[k]) < -clip:  # modif du 17 mai 2025 profil Aguerre 8 bits
                    if abs(fp[k]) >clip :
                        #print("kk ",kk)
                        del bords[i][kk]
                        del bordsY[i][kk]
...
def auto_crop_img (cam_height, h,w, frame, cercle0, debug_crop, param,flag_h20):
    
    debug_crop=False
    verbose = False
    
    # si on force 20 pourcent de plus en largeur
    if flag_h20  :
        ext20=1
    else:
        ext20 =0
    ...
    if asym_h <0  or asym_h <= cercle0[2]+0: # was 20 sur 6.2
        if verbose : print("on ne centre pas en hauteur")
        if crop_force_largeur != 0 :
            crop_wi = int(crop_force_largeur)
            if verbose : print("autocrop forced")
        else :
            # largeur image multiple de ih
            # version 5.0 etait à 50
            if diam_sol+100 >= 2*ih :
                if verbose :print("on prend largeur image a 3*ih"+ str(3*ih))
                crop_wi = int(ih*3*(1+ext20*0.2))-int(ih*3*(1+ext20*0.2))%2
            else  :
                if diam_sol +100 >=ih :
                    if verbose :print("on prend largeur image a 2*ih"+ str(2*ih))
                    crop_wi = int(ih*2*(1+ext20*0.2))-int(ih*2*(1+ext20*0.2))%2
                else :
                    if verbose :print("on garde image carrée")
                    crop_wi = int(crop_he*(1+ext20*0.2))-int(crop_he*(1+ext20*0.2))%2
                    #print(crop_wi)
        ...
        # modif 4 juillet 25
        if 2*cercle0[2]> crop_he : 
            d_vert = 0 # avant c'etait toujours le cas
            print("on ne centre pas en hauteur")
        else :
            # modif ajout 4 juillet 25
            d_vert=int(cercle0[1])-int(crop_he//2)
    ...
    # on gere les decalages horizontaux pour tous les cas
    ...
    """
    # gere le padding droit et gauche
    ...
    if h-crop_he+dcv_top < 0 : # doit faire du padding  
        if crop_he+dv_top > h :
            if d_vert <0 :
                dcv_bas=h+dcv_top
                dv_bas=h
    ...
    if verbose : print('crop image (h,w) : ' + str( crop_img.shape))
...
def translate_img (img_mean, poly):
    ...
    img_mean_redresse = np.zeros((ih,iw), dtype='uint16')
    ...
    for y in range(0,ih):
        ...
        f=interp1d(np.arange(0,iw)-ecart[y],myline,kind='linear',fill_value=0, bounds_error=False)
